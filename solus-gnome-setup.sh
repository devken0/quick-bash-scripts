...
sudo eopkg install scrcpy borg vorta chrome-gnome-shell libreoffice-draw virt-manager burp openjdk-17 filezilla gcolor3 keepassxc etcher cheese clipit gufw vlc android-tools wget deja-dup grsync vim git pavucontrol geary brave geany flameshot peek obs-studio tigervnc htop neofetch yt-dlp ncdu simplenote shotcut kdenlive qbittorrent dropbox discord telegram kdeconnect linux-current-headers linux-lts-headers codeblocks thonny meld openssh-server gnome-screenshot
